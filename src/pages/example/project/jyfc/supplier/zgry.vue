    ...
    <div style="overflow:hidden;">
        <backbtn v-bind:route-bool="true" v-bind:route-name="'UserIndex'"></backbtn>
            ...
            <div v-if="!dataNull" style="margin-top: 50px;" v-infinite-scroll="loadMore"
                ...
                <div  class="worker bcontainer1 padding30lr padding30tb line-space"  v-for="(item,key,index) in list" v-bind:key="index" @click="detail(item.id)">
  ...
  watch: {
    name (val, oldVal) {
      ...
      this.timer = setTimeout(() => {
        // TODO此处写逻辑
        this.getSupplierEpibolyStaffList(2)
      }, 200)
    }
  },
  methods: {
    loadMore: function () {
      if (this.page > 1) {
        this.getSupplierEpibolyStaffList()
      }
    },
    getSupplierEpibolyStaffList: function (v) {
      this.loading = true
      var apiUrl = this.GLOBAL.requestHost + this.GLOBAL.requestList.getSupplierEpibolyStaffList.path
      var obj
      if (v === 2) {
    ...
    detail: function (id) {
      this.$router.push({name: 'SupGwxx', params: {id: id}})
    }
  },
  mounted: function () {
    this.getSupplierEpibolyStaffList()
