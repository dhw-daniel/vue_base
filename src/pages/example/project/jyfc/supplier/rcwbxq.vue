                    ...
                    <div class="worker bcontainer1 top-space30 padding30lr padding30tb" v-for="(item,key,index) in info.dataList" v-bind:key="index">
                        <div class="info prop">
                            <ul>
                                ...
                                <li><span>岗位级别</span>{{item.gradeName}}<span class="linkdiv" @click="show1(item.id,info.fid)">[查看具体要求]</span></li>
                        ...
                        <div class="borderspace top-space50 padding30lr line-space">
                            <div class="btncolor1" v-if="status==1"><mt-button size="large" type="primary" @click.Native="bm(item.id,info.fid)">报名</mt-button></div>
                            <div class="btncolor2" v-if="status==2"><mt-button size="large" type="primary" @click.Native="bmover">报名已结束</mt-button></div>
                            <div class="btncolor2" v-if="status==0"><mt-button size="large" type="primary">报名未开始</mt-button></div>
                        </div>
                    </div>
                </div>
...
export default {
  ...
  methods: {
    show1: function (id, fid) {
      this.$router.push({name: 'SupGwjtyq', params: {id: id, fid: fid}})
    },
    show2: function (t, v) {
      this.title = t
      this.content = v ? v : '无'
    ...
    bm: function (id, fid) {
      this.$router.push({name: 'SupRcwbbm', params: {id: id, fid: fid}})
    },
    bmover: function () {
      this.$toast({message: '报名已结束', position: 'middle', duration: 1000})
    },
    getText: function (v) {
      this.pd = v
    ...
    getSupplierDemandTalentInfo: function () {
      var apiUrl = this.GLOBAL.requestHost + this.GLOBAL.requestList.getSupplierDemandTalentInfo.path
  ...
  mounted: function () {
    ...
    this.getSupplierDemandTalentInfo()
  }
}
</script>
<style>
