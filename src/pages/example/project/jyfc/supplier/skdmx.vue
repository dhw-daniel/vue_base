<template>
    <div>
        ...
        <div class="needo bcontainer2 padding30lr padding30tb line-space" >
            <div class="status">{{statusName[info.status]}}</div>
        ...
        <div class="padding30lr padding30tb ">收款单明细</div>
...
export default {
  ...
  data () {
    return {
      ...
      popupVisible: false,
      info: '',
      statusName: ['', '待甲方确认', '甲方有异议', '待开票', '待甲方付款', '已退票,待重开', '甲方已付款,待确认', '已收款', '已作废']
    }
  },
  methods: {
    show: function (v) {
    ...
    getSupplierSettleSkDetail: function () {
      var apiUrl = this.GLOBAL.requestHost + this.GLOBAL.requestList.getSupplierSettleSkDetail.path
      var obj = {id: this.id}
  ...
  mounted: function () {
    ...
    this.getSupplierSettleSkDetail()
  }
}
</script>
