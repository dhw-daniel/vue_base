    ...
    <div class="searchdiv top-space30">
        ...
        <div class="top-space30" v-infinite-scroll="loadMore"
            ...
            <div v-if="!dataNull" class="jso bcontainer2 padding30lr padding30tb line-space" v-for="(item,key,index) in list" v-bind:key="index">
                <div class="more" @click="viewdetial(item.id,item.isAttence)">详情</div>
                <div class="user" @click="viewdetial(item.id,item.isAttence)">{{item.name}}<span v-if="item.isConfirm" class="status1">[已确认]</span><span v-else class="status1">[待确认]</span><span class="status2">[{{d2[0].values[item.jsStatus+1]}}]</span></div>
                ...
                <div class="btngroup">
                    <div class="op borderspace top-space30 btncolor1" v-if="true">
                        <mt-button size="large" type="primary" @click.native="queren(item.id,item.name,item.time,item.num,item.amt,item.qtAmt)">编辑工作量</mt-button>
                    </div>
                    <div class="op borderspace top-space30 btncolor2" v-if="item.yy!=''">
                        <mt-button size="large" type="primary"  @click.native="chakan(item.yy)">查看异议</mt-button>
                    </div>
                    <div class="op borderspace top-space30 btncolor3" v-if="item.isAttence">
                      <mt-button size="large" type="primary"  @click.native="showAttendance(item.id)">查看考勤</mt-button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    queren: function (id, name, time, num, amt, qtAmt) {
      this.$router.push({name: 'PurBjgzl', params: {id, name, time, num, amt, qtAmt}})
    },
    chakan: function (yy) {
      this.yy = yy ? yy : '暂无'
    ...
    viewdetial: function (id, isAttence) {
      this.$router.push({name: 'PurGztjxq', params: {id: id, isdata: isAttence}})
    },
    showAttendance: function (id) {
      this.$router.push({name: 'PurCkkq', params: {id: id}})
    },
    getPurchaserWrtList: function () {
      this.loading = true
      var apiUrl = this.GLOBAL.requestHost + this.GLOBAL.requestList.getPurchaserWrtList.path
