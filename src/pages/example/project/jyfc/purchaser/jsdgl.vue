            ...
            <mt-tab-container v-model="selected">
                <mt-tab-container-item id="0">
                    <div v-if="!dataNull" class="bcontainer ck1 padding30lr padding30tb line-space" v-for="(item,key,index) in list" v-bind:key="index">
                        <div class="prop">
                            <div class="cks" v-if="false"><input type="checkbox" class="input-check" :value="item.id" :id="item.id" v-model="ids"></div>
                            <ul>
                                <li><span class="">状态</span><span class="b">待结算</span></li>
                                <li><span class="">金额</span><strong>{{item.amt}}</strong></li>
                                <li v-if="item.isSg"><span class="">类型</span>{{type_name[item.type]}}[手工生成]</li>
                                <li v-else><span class="">类型</span>{{type_name[item.type]}}</li>
                                <li v-if="item.type==1 && item.isSg == false"><span class="">人员</span>{{item.name}}</li>
                                <li v-if="item.type==2 && item.isSg == false"><span class="">名称</span>{{item.gdName}}</li>
                                <li v-if="item.type ==1 &&item.isSg == false"><span class="">工作量时间</span>{{item.time}}</li>
                                <li><span class="spanfont">结算单生成日期</span>{{item.ctime}}</li>
                                <li v-if="item.jsStatus>0"><span class="spanfont">生成付款单日期</span>{{item.fkTime}}</li>
                                <li v-if="item.type ==1 &&item.isSg == false"><span class="">工作量</span>{{item.num}}人月</li>
                                <li><span class="">结算单编号</span>{{item.sn}}</li>
                                <li><span v-if="item.type==2" class="">工单编号</span><span v-else="">工作量编号</span>{{item.tsn}}</li>
                                <li v-if="item.jsStatus>0"><span class="">付款单编号</span>{{item.fkSn}}</li>
                        ...
                        <div class="vb top-space30 btncolor3"><mt-button size="large" type="primary" @click.Native="getlink(item.id, item)">我有异议</mt-button></div>
                    </div>
                ...
                <mt-tab-container-item id="1">
                    <div v-if="!dataNull" class="bcontainer ck1 padding30lr padding30tb line-space" v-for="(item,key,index) in list" v-bind:key="index">
                        <div class="prop">
                            <ul>
                              <li><span class="">状态</span><span class="b">结算中</span><span class="b" style="font-size: 14px">[{{item.fkMemo}}]</span><span class="r">{{item.skMemo}}</span></li>
                              <li><span class="">金额</span><strong>{{item.amt}}</strong></li>
                              <li v-if="item.isSg"><span class="">类型</span>{{type_name[item.type]}}[手工生成]</li>
                              <li v-else><span class="">类型</span>{{type_name[item.type]}}</li>
                              <li v-if="item.type==1 && item.isSg == false"><span class="">人员</span>{{item.name}}</li>
                ...
                <mt-tab-container-item id="2">
                    <div v-if="!dataNull" class="bcontainer ck1 padding30lr padding30tb line-space" v-for="(item,key,index) in list" v-bind:key="index">
                        <div class="prop">
                            <ul>
                              <li><span class="">状态</span><span class="b">已结算</span></li>
                              <li><span class="">金额</span><strong>{{item.amt}}</strong></li>
                              <li v-if="item.isSg"><span class="">类型</span>{{type_name[item.type]}}[手工生成]</li>
                              <li v-else><span class="">类型</span>{{type_name[item.type]}}</li>
                              <li v-if="item.type==1 && item.isSg == false"><span class="">人员</span>{{item.name}}</li>
...
export default {
  ...
  methods: {
    ...
    getlink: function (v, item) {
      this.$router.push({name: 'PurJsdyyy', params: item})
    },
    getPurchaserSettleList: function () {
      this.loading = true
      var apiUrl = this.GLOBAL.requestHost + this.GLOBAL.requestList.getPurchaserSettleList.path
